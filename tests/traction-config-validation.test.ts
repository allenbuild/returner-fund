...
describe("traction scoring config validation", () => {
  it("accepts the canonical v4 config without mutating it", () => {
    ...
    expect(config).not.toHaveProperty("momentumSignalWeight");
    expect(config).not.toHaveProperty("missingDateMomentum");
    for (const reference of Object.values(config.platformReferences)) {
      expect(reference).not.toHaveProperty("halfLifeDays");
    }
  });
  ...
  it.each<[string, ConfigMutation, RegExp]>([
    ...
    [
      "a normalized strongest-platform override",
      (config) => {
    ...
    [
      "a normalized cohort calibration blend",
      (config) => {
        config.batchCalibration.absoluteScoreWeight = 0.8;
        config.batchCalibration.cohortPercentileWeight = 0.2;
      },
      /absolute company score must disable cohort calibration/
    ],
    [
