...
async function main(argv = process.argv.slice(2)) {
  ...
  const result = await generateHistoricalCompletionProofs({
    ...
    recencyCutoffAt: options.recencyCutoffAt,
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (flag === "--recency-cutoff-at") options.recencyCutoffAt = value;
...
function helpText() {
  return `Usage: node scripts/generate-historical-completion-proofs.mjs [options]\n\n` +
    ...
    `  --recency-cutoff-at=<ISO UTC>   Immutable recent cutoff shared with the campaign, if different from generatedAt minus 90 days.\n` +
