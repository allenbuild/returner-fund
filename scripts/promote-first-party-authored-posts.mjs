...
import { planFirstPartyAuthoredPostPromotion } from "./lib/first-party-authored-post-promotion.mjs";
import {
...
export function parseFirstPartyPromotionArgs(rawArgs) {
...
export async function promoteFirstPartyAuthoredPosts(
  rawArgs = process.argv.slice(2),
  dependencies = {}
) {
  const args = parseFirstPartyPromotionArgs(rawArgs);
  ...
  const [canonicalArtifact, candidateBytes, graphDocuments, referenceDocuments] = await Promise.all([
    readPublicEvidenceArtifact(canonicalPath, { rootDir, readFileImpl }),
    readFileImpl(candidatePath),
    Promise.all(GRAPH_PATHS.map((relativePath) => readJson(path.join(rootDir, relativePath), readFileImpl))),
    Promise.all(REFERENCE_PATHS.map((relativePath) => readJson(path.join(rootDir, relativePath), readFileImpl)))
  ]);
  const candidateSha256 = sha256(candidateBytes);
  if (candidateSha256 !== args.candidateSha256) {
    throw new Error(`Candidate SHA-256 mismatch: expected ${args.candidateSha256}, received ${candidateSha256}.`);
  }
  const candidate = parseJson(candidateBytes, candidatePath);
  const plan = planFirstPartyAuthoredPostPromotion({
  ...
  assertExpected(plan.resolvedReview.length, args.expectedResolvedReview, "resolved review rows");
  const receipt = {
    schemaVersion: 1,
...
async function readJson(sourcePath, readFileImpl) {
  return parseJson(await readFileImpl(sourcePath), sourcePath);
}
...
if (isMain) {
  promoteFirstPartyAuthoredPosts().catch((error) => {
