...
export async function generateHistoricalCompletionProofs({
  ...
  outputDir = null,
  generatedAt,
  recencyCutoffAt: requestedRecencyCutoffAt = null,
  dryRun = false,
  ...
  const recencyCutoffAt = requestedRecencyCutoffAt
    ? canonicalTimestamp(requestedRecencyCutoffAt, "recencyCutoffAt")
    : new Date(
        Date.parse(normalizedGeneratedAt) -
          INGESTION_RECENCY_WINDOW_DAYS * 24 * 60 * 60 * 1_000
      ).toISOString();
  if (Date.parse(recencyCutoffAt) > Date.parse(normalizedGeneratedAt)) {
    throw new Error("recencyCutoffAt cannot be later than generatedAt.");
  }
