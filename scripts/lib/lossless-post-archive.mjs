...
export class LosslessPostArchive {
  ...
  async appendPost(input) {
    await this._ready;
    const request = preparePostRequest(input, this.clock);

    return this._enqueue(async () => {
      this._assertPostCompatibility(request.normalizedRecord);
  ...
  async _load() {
    ...
    await this._loadFile(ARCHIVE_FILES.rawEnvelopes, (record) => {
      const fingerprint = `${record.key}\u001f${record.contentHash}`;
      this._indexes.rawHashes.add(fingerprint);
      this._indexes.rawEnvelopes.push(record);
    });
    await this._loadFile(ARCHIVE_FILES.normalizedPosts, (record) => {
      const existing = this._indexes.posts.get(record.key);
      if (existing && existing.contentHash !== record.contentHash) {
        throw conflict("normalized_post", record.key, null, existing.contentHash, record.contentHash);
      }
      this._indexes.posts.set(record.key, existing ?? record);
    });
  ...
  _assertPostCompatibility(record) {
    const existing = this._indexes.posts.get(record.key);
    if (existing && existing.contentHash !== record.contentHash) {
      throw conflict("normalized_post", record.key, null, existing.contentHash, record.contentHash);
    }
  ...
  async _appendRawIfNew(record) {
    const fingerprint = `${record.key}\u001f${record.contentHash}`;
    if (this._indexes.rawHashes.has(fingerprint)) return result("duplicate", record);
...
function preparePostRequest(input, clock) {
  ...
  const normalizedPost = normalizePost(normalizedInput, platform, nativeId);
...
function normalizePost(post, platform, nativeId) {
  const normalized = { ...cloneJson(post), platform, nativeId };
  if (normalized.media === undefined) normalized.media = [];
  if (!Array.isArray(normalized.media)) throw new TypeError("normalizedPost.media must be an array");

  const suppliedRelationships = normalized.relationships ?? {};
  normalized.relationships = {
    parent: suppliedRelationships.parent ?? normalized.parent ?? normalized.parentId ?? null,
    thread: suppliedRelationships.thread ?? normalized.thread ?? normalized.threadId ?? null,
    quote: suppliedRelationships.quote ?? normalized.quote ?? normalized.quoteId ?? null
  };
  return normalized;
}
