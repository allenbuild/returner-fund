...
function selectCurrentAttempt(outcomes, taskKey) {
  ...
  for (let index = 1; index < ordered.length; index += 1) {
    ...
    if (previous.attemptSequence < current.attemptSequence &&
        (previous.checkedAt > current.checkedAt || previous.startedAt > current.startedAt)) {
      throw new Error(`${taskKey} attempt sequence and checkedAt ordering disagree.`);
    }
  }
  return ordered.at(-1);
}
...
function rawReasonSignals(reason) {
  ...
  if (/(?:rate.?limit|\b429\b)/i.test(text)) signals.add("rate_limited");
...
function assertObservedWithinAttempt(value, attempt, label) {
  ...
  if (
    ...
    throw new Error(`${label} must correlate with its current attempt window.`);
  }
}
