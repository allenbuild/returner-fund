...
export async function generateRecentCompletionProofs({
  ...
  const campaignGeneratedAt = canonicalTimestamp(manifest.generatedAt, "campaign.generatedAt");
  const coverageGeneratedAt = canonicalTimestamp(
    manifest.coverageGeneratedAt,
    "campaign.coverageGeneratedAt"
  );
  if (coverageGeneratedAt > campaignGeneratedAt) {
    throw new Error("campaign.coverageGeneratedAt cannot exceed campaign.generatedAt.");
  }
  // Proof manifests are bound to the immutable coverage read, not the later
  // packaging/materialization timestamp. This keeps historical, recent, and
  // materialization artifacts on one auditable read timestamp.
  const generatedAt = coverageGeneratedAt;
