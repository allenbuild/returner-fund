...
function crossValidateProofs({
  ...
  const status = projection.fullIngestionCoverageStatus;
  if (historicalManifest.generatedAt !== projection.generatedAt ||
      recentManifest.generatedAt !== projection.generatedAt) {
    throw new Error("Proof manifests and materialization do not share one read timestamp.");
  }
...
function validateRecentProofArtifacts({ proofs, exclusions, manifest }) {
  ...
  for (const [index, proof] of proofs.entries()) {
    ...
    if (proof.receipt.status !== "complete" ||
        ...
        proof.receipt.checkedAt !== manifest.window.coveredThrough) {
