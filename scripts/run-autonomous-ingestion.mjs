...
// consume runCommand's default 40 KiB tail capture.
const LINKEDIN_REPLAY_PLAN_CAPTURE_LIMIT = 16 * 1024 * 1024;
...
await Promise.all([
  ...
  if (run?.status === "completed") {
    ...
    if (!args.skipNetwork && args.authenticatedSocialReplay) {
      await event(
        ...
        "Authenticated social historical replay started with bounded platform-specific parallelism.",
        {
          ...
          requestedScope: args.authenticatedBackfillScope,
          requestedTarget: args.authenticatedBackfillTarget,
          requestedPlatforms: authenticatedBackfillPlatformsForScope(
            args.authenticatedBackfillScope
          ),
          xWorkers: 3,
          instagramWorkers: 2,
          linkedinWorkers: 1,
    ...
    if ((args.skipNetwork || args.authenticatedSocialReplay) && run) {
      await terminalizeQueuedTasks(
        run.id,
        "skipped",
        args.authenticatedSocialReplay
          ? "public_collector_matrix_not_applicable_authenticated_replay"
          : "network_collection_explicitly_skipped"
      );
    ...
    collectionCoverage.collectorValidation = args.authenticatedSocialReplay
      ? {
          status: "not_applicable",
          reason: "authenticated_replay_does_not_run_public_or_github_collector_matrix"
        }
      : { status: "required", reason: null };
...
async function runAuthenticatedCollectors({
  ...
  if (!authenticatedBackfillPlatformsForScope(requestedScope)) {
    throw new Error("Authenticated collector scope must be all, x, instagram, or linkedin.");
  }
  if (requestedTarget) {
    const validatedTarget = resolveAuthenticatedBackfillTarget({
      authenticatedReplay: historicalReplay,
      requestedScope,
      batchSlug: requestedTarget.batchSlug,
      companySlug: requestedTarget.companySlug
    });
    if (!authenticatedBackfillTargetEquals(validatedTarget, requestedTarget)) {
      throw new Error("Authenticated collector target must be an exact canonical target.");
    }
  }
  const requestedPlatforms = authenticatedBackfillPlatformsForScope(requestedScope);
  if (!requestedPlatforms) {
    throw new Error("Authenticated collector scope must be all, x, instagram, or linkedin.");
  }
  const selectedBatches = requestedTarget
    ? AUTONOMOUS_BATCHES.filter((batch) => batch.slug === requestedTarget.batchSlug)
    : AUTONOMOUS_BATCHES;
  if (requestedTarget && selectedBatches.length !== 1) {
    throw new Error("Authenticated collector target did not resolve to exactly one configured batch.");
  }
  ...
  try {
    const candidate = await runAuthenticatedSocialRunnerPreflight({
      env: {
        ...process.env,
        AUTHENTICATED_SOCIAL_REPLAY: historicalReplay ? "true" : "false",
        AUTHENTICATED_BACKFILL_SCOPE: requestedScope,
        AUTHENTICATED_BACKFILL_BATCH: requestedTarget?.batchSlug ?? "all",
        AUTHENTICATED_BACKFILL_COMPANY_SLUG: requestedTarget?.companySlug ?? ""
      }
    });
    ...
    if (
      ...
      !authenticatedBackfillTargetEquals(candidate.requestedTarget ?? null, requestedTarget)
    ) {
      throw new Error("Authenticated social preflight returned mismatched scope or target evidence.");
    }
    preflight = candidate;
  } catch {
    const configured = [
      ...
      "OPENCLI_HOME",
      "OPENCLI_PROFILE",
      ...(requestedPlatforms.includes("linkedin")
        ? ["RETURNER_LINKEDIN_VIEWER_PROFILE"]
        : []),
    ...
    preflight = {
      ...
      x: requestedPlatforms.includes("x")
        ? { ...unavailable }
        : unrequestedAuthenticatedPlatform(),
      instagram: requestedPlatforms.includes("instagram")
        ? { ...unavailable }
        : unrequestedAuthenticatedPlatform(),
      linkedin: requestedPlatforms.includes("linkedin")
        ? { ...unavailable }
        : unrequestedAuthenticatedPlatform()
    };
  }
  const readiness = {
    ...
    service: preflight.service
      ...
      : null,
    x: {
      ok: preflight.x?.ok === true,
      reason: preflight.x?.reason,
      attempts: preflight.x?.attempts ?? 0
    },
    instagram: {
  ...
  if (!preflight.configured) {
    ...
    const skippedReplayState = reduceLinkedInReplayState(
      createLinkedInReplayState(
        selectedBatches.map((batch) => batch.slug),
  ...
  const batches = [];
  const xRequested = requestedPlatforms.includes("x");
  const xReady = xRequested && preflight.x?.ok === true;
  ...
  const linkedinRequested = requestedPlatforms.includes("linkedin");
  const linkedinPreflightReady = linkedinRequested && preflight.linkedin?.ok === true;
  const durableLinkedinLockReady = !linkedinRequested || Boolean(
    cleanEnv(process.env.NEXT_PUBLIC_SUPABASE_URL) &&
    cleanEnv(process.env.SUPABASE_SERVICE_ROLE_KEY) &&
    cleanEnv(process.env.LINKEDIN_GLOBAL_LOCK_NAMESPACE)
  );
  if (historicalReplay && linkedinRequested && requestedScope === "linkedin" && !durableLinkedinLockReady) {
    throw new Error(
  ...
  for (const batch of selectedBatches) {
    ...
    const x = xReady
      ...
      : {
          status: "skipped",
          reason: xRequested
            ? "authenticated_x_preflight_failed"
            : "authenticated_platform_not_requested",
          preflightReason: readiness.x.reason
        };
    ...
    let linkedin;
    if (!linkedinRequested) {
      linkedin = {
        status: "skipped",
        reason: "authenticated_platform_not_requested"
      };
    } else if (!linkedinPreflightReady) {
      linkedin = {
        status: "skipped",
    ...
    if (linkedinRequested && linkedinPreflightReady && !durableLinkedinLockReady) {
      await event(
    ...
    batches.push({ batchSlug: batch.slug, x, instagram, linkedin });
  }
  const baseLinkedinReplay = createLinkedInReplayResult({
    ...replayState,
    status: !linkedinRequested
      ? "not_applicable"
      : !historicalReplay
      ? "not_applicable"
  ...
  const platformStatus = {
    x: {
      requested: xRequested,
      status: !xRequested
        ? "not_requested"
        : batches.every(({ x }) => x.status === "completed")
          ? "completed"
          : "incomplete"
    },
    instagram: {
...
function unrequestedAuthenticatedPlatform() {
  return {
    ok: false,
    requested: false,
    skipped: true,
...
async function runAuthenticatedLinkedInReplayBatch({
  ...
  while (!state.halted) {
    ...
    const plan = await runAuthenticatedLinkedInPlan(
      batch.slug,
      linkedinArgs,
      { deadlineAt: childDeadlineAt, requestedTarget }
    );
...
async function runAuthenticatedLinkedInPlan(
  batchSlug,
  linkedinArgs,
  { deadlineAt, requestedTarget = null } = {}
) {
  ...
  try {
    ...
    assertAuthenticatedLinkedInPlanTarget(plan, requestedTarget);
...
async function runAuthenticatedCollectorCommand(
  ...
  try {
    const result = await runCommand(process.execPath, [
      ...
      quiet: planOnly,
      captureLimit: planOnly ? LINKEDIN_REPLAY_PLAN_CAPTURE_LIMIT : 40_000,
      requireCompleteOutput: planOnly,
      cwd: root
    });
...
async function recordCollectionCoverage(
  ...
  const collectorValidation = coverage.collectorValidation ?? {
    status: "required",
    reason: null
  };
  const summary = {
    ...
    terminalFailureBudget,
    status: collectorValidation.status === "not_applicable"
      ? "not_applicable"
      : budgetExceeded
  ...
  await appendFile(githubSummary, [
    ...
    ...(collectorValidation.status === "not_applicable"
      ? [`- Collector validation: not applicable (${collectorValidation.reason})`]
      : []),
...
function assertAuthenticatedReplayCanPublish(replay) {
  ...
  const requestedTarget = replay.requestedTarget ?? null;
  if (!authenticatedBackfillTargetEquals(linkedinReplay.requestedTarget ?? null, requestedTarget)) {
    throw new Error("Authenticated replay target receipt does not match its bounded LinkedIn replay receipt.");
  }
  if (requestedTarget) {
    const validatedTarget = resolveAuthenticatedBackfillTarget({
    ...
    if (!authenticatedBackfillTargetEquals(validatedTarget, requestedTarget)) {
      throw new Error("Authenticated replay target receipt is not canonical.");
    }
    const expectedBatchKeys = [requestedTarget.batchSlug];
    const replayBatchKeys = Object.keys(linkedinReplay.remainingByBatch ?? {});
    const batches = replay.batches ?? [];
    if (
      JSON.stringify(replayBatchKeys) !== JSON.stringify(expectedBatchKeys) ||
      batches.length !== 1 ||
      batches[0]?.batchSlug !== requestedTarget.batchSlug ||
      (
        expectedPlatforms.includes("linkedin") &&
        !authenticatedBackfillTargetEquals(
          batches[0]?.linkedin?.finalPlan?.requestedTarget ?? null,
          requestedTarget
        )
      )
    ) {
  ...
  for (const batch of replay.batches ?? []) {
    for (const platform of ["x", "instagram", "linkedin"]) {
      ...
      if (
        batch?.[platform]?.status !== "skipped" ||
        batch?.[platform]?.reason !== "authenticated_platform_not_requested"
      ) {
        throw new Error(
          `Authenticated replay receipt indicates that an unrequested ${platform} collector was invoked.`
        );
      }
    }
  }
  ...
  if (replay.historicalReplay && replay.status === "partial") {
    const incompleteNonLinkedInPlatforms = incompleteRequestedPlatforms.filter(
      (platform) => platform !== "linkedin"
    );
    if (incompleteNonLinkedInPlatforms.length > 0) {
      throw new Error(
        `Authenticated historical replay cannot publish while requested non-LinkedIn platforms are incomplete: ` +
        `${incompleteNonLinkedInPlatforms.join(", ")}.`
      );
    }
    if (
      linkedinReplay.configurationSkipped ||
      linkedinReplay.safetyStopped ||
      linkedinReplay.infrastructureStopped
    ) {
      throw new Error(
        "Authenticated historical replay cannot publish after a LinkedIn configuration, safety, or infrastructure stop."
      );
    }
    const boundedCheckpointDebt =
      (linkedinReplay.chunkBudgetExhausted || linkedinReplay.deadlineExhausted) &&
      linkedinReplay.chunksCompleted > 0;
    if (!boundedCheckpointDebt) {
      throw new Error(
        "Authenticated historical replay cannot publish a partial result without completed chunks and bounded checkpoint debt."
      );
    }
  }
  if (
  ...
  if (
    ...
    linkedinReplay.status !== "completed"
  ) {
...
async function completePublishedTimelineInvalidations(
  ...
  if (
    timelineBuildReceipt?.status !== "rebuilt" ||
    timelineBuildReceipt?.mode !== "database_backed"
  ) {
    await event(
      "timeline.invalidations.deferred",
      "warning",
      "Timeline artifact invalidations remain open because this publication did not include the durable Timeline database projection.",
      {
        ...
        timelineBuildMode: timelineBuildReceipt?.mode ?? "unknown",
        publishedCommit: publicationReceipt.publishedCommit
      }
    );
    return;
...
async function buildAndValidatePublication(publicationRunId, catalogState) {
  ...
  const coverageMigrationUnavailable =
  ...
  const timelineUsesDatabase = durableStorageConfigured && !coverageMigrationUnavailable;
  const timelineDatabaseSnapshotPath = join(workRoot, "timeline-database-snapshot.json");
  if (coverageMigrationUnavailable) {
    await event(
      "timeline.artifacts.file_backed_fallback",
      "warning",
      "Company Timeline coverage migration is unavailable; rebuilding every Timeline artifact from the fresh canonical graph and evidence inventory without a database read or write.",
      {
        reason: timelineDiscoveryReceipt.reason,
        mode: "file_backed"
      }
    );
  }
  if (timelineUsesDatabase) {
    await runCommand(process.execPath, [
      "--experimental-strip-types",
      "--loader",
      sourcePath("scripts", "lib", "scoring-diagnostics-ts-loader.mjs"),
  ...
  latestTimelineBuildReceipt = {
    status: "rebuilt",
    mode: timelineUsesDatabase ? "database_backed" : "file_backed",
    reason: coverageMigrationUnavailable
      ? "timeline_coverage_migration_unavailable_file_backed_fallback"
      : "timeline_discovery_and_backfill_completed"
  };
  await runCommand(process.execPath, [
    sourcePath("scripts", "prepare-graph-runtime-evidence.mjs"),
    `--root=${targetRoot}`
...
async function writeRunnerOutcome(outcome) {
  ...
  const outputs = {
    ...
    authenticated_backfill_target_batch: normalized.authenticatedSocialReplay?.requestedTarget?.batchSlug ?? "",
    authenticated_backfill_target_company_slug: normalized.authenticatedSocialReplay?.requestedTarget?.companySlug ?? "",
...
function parseArgs(rawArgs) {
  ...
  if (!authenticatedBackfillPlatformsForScope(authenticatedBackfillScope)) {
    throw new Error("--authenticated-backfill-scope must be all, x, instagram, or linkedin.");
  }
  const authenticatedSocialReplay = booleanValue("--authenticated-social-replay");
  const authenticatedBackfillTarget = resolveAuthenticatedBackfillTarget({
    authenticatedReplay: authenticatedSocialReplay,
    requestedScope: authenticatedBackfillScope,
    batchSlug: value("--authenticated-backfill-batch") ?? "all",
    companySlug: value("--authenticated-backfill-company-slug") ?? ""
  });
  return {
