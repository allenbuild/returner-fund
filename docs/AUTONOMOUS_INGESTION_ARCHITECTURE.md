...
Public schedules, recovery dispatches, and manual replays without `authenticated_backfill=true` run on GitHub-hosted Linux; only an explicit authenticated backfill uses the labeled self-hosted Mac. The accepted-slot step wraps the coordinator with a same-key retry controller for explicit transient transport and timeout failures. A child is started only when at least 331 minutes remain: the coordinator's 324-minute work budget, its full six-minute cleanup allowance, and one minute for process startup. The controller never shortens that child window. Hosted runs use a 335-minute controller budget inside a 342-minute step and 360-minute job; authenticated Mac replays retain a 370-minute controller budget inside a 380-minute step and 415-minute job. A failed or cancelled hosted collection saves only its public collector recovery directory to an immutable, source-SHA-bound Actions cache whenever the runner remains available for post-steps. The next same-slot wakeup restores the newest compatible cache before collection, so an ephemeral VM replacement does not discard completed checkpoints or validated terminal snapshots. Authenticated browser state and `OPENCLI_HOME` are never cached. Later scheduled watermark wakeups continue retrying a stale slot when a same-job retry cannot safely receive that complete window. Because GitHub documents that scheduled events may be delayed or dropped, the installed five-minute host supervisor independently checks for a 30-minute workflow silence, no active or pending ingestion, and a stale committed watermark without consulting Mac runner, power, or wake state. It then sends only the trusted `autonomous-ingestion-recovery` event with the inspected `main` SHA; the workflow recomputes the canonical newest slot and watermark from that exact commit and never accepts a host-supplied slot key.
...
If a completed run already exists for the key, the invocation is a no-op. An incomplete run with the same key reacquires its run lease before resuming. Snapshot recovery is enabled by default. In GitHub Actions, collector state is kept outside the checkout-cleaned repository under `RETURNER_INGESTION_STATE_ROOT`, `OPENCLI_HOME`, or `RUNNER_WORKSPACE` (in that order). A self-hosted replacement invocation reuses that durable host directory directly. A failed GitHub-hosted public invocation saves the configured state root to Actions cache, and only a later Linux invocation with the same slot key and source SHA may restore it. The cache is a bounded recovery accelerator, not publication evidence or the durable source of truth; every reused snapshot still passes the coordinator's provenance, freshness, and referential-integrity validation.
...
| X | Companies and founders in all three batches | `X_BEARER_TOKEN` drives batched official recent-search reads for exact mapped accounts; public reader/search remains a fallback | The official recent-search window is bounded. A missing token, partial API response, or owner group with more than 100 recent posts degrades the run instead of being reported healthy. |
...
| LinkedIn | Companies and founders in all three batches | `EXA_API_KEY` adds domain-restricted native-post discovery; strict native-author/body/metric attribution still controls acceptance | Exa discovers public URLs but does not bypass LinkedIn login walls. Candidates without independently sufficient native attribution remain review items. |
| YouTube | Company collection in all three batches | Public search plus exact embedded-video extraction from official YC company pages | The planner marks founder YouTube tasks not applicable. Embedded videos still require positive native metrics and native channel identity before acceptance. |
| Product Hunt | Company collection in all three batches | Public pages/search, official YC-page link extraction, and domain-restricted Exa search | Product pages can still block; candidates require exact company/domain attribution. Founder tasks are not applicable. |
| Reddit | Company collection in all three batches | Current collection uses public search JSON; the reader page only diagnoses access blocks, and the workflow does not request unused Reddit credentials | Network restrictions can block the JSON request. There is no page-parsing or OAuth collection fallback in the current collector. |
