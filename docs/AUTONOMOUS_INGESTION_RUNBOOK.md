...
The same five-minute supervisor also closes the Dashboard's dropped-hourly-event gap. It reads and validates the publication envelope in `public/dashboard/feed.json` from the exact current `main` SHA and, only when that watermark is more than two hours old (or the artifact is missing or invalid), checks that both Autonomous Ingestion and Technology Dashboard Refresh have no active or pending run, the exact runner is online and idle, and the Mac is on AC. It then dispatches the existing Dashboard workflow on `main` with `skip_external_discovery=false`. Failed, cancelled, and skipped Dashboard runs do not count as freshness and do not consume durable state; only an accepted supervisor dispatch starts the 30-minute cooldown. A recently completed successful Dashboard wake receives a 30-minute silence window because a broad-source outage may deliberately retain the older truthful watermark. Recovery therefore preserves the full YouTube and Instagram source-proof gates and never advances freshness by substituting the public-only replay mode.
