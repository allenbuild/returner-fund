...
Independently, after 30 minutes without any autonomous workflow wakeup, the supervisor verifies that the workflow is active, no autonomous run is active or pending, and the complete publication watermark read from the current `main` SHA is stale, missing, invalid, or divergent. It then emits `autonomous-ingestion-recovery` with only that expected SHA. Public recovery runs on GitHub-hosted capacity and never inspects the Mac runner, battery, or wake state; the Mac gates remain limited to authenticated replay and repair operations. The workflow requires the dispatched SHA to match, rereads the committed watermark, and computes the newest `06:00`/`18:00` Central slot itself. A durable same-slot 30-minute cooldown and GitHub active-run check prevent dispatch storms. Refresh the installed host services after merging installer or supervisor changes; merging alone does not update the copied host script or loaded plists.

The same five-minute supervisor also closes the Dashboard's dropped-hourly-event gap. It reads and validates the publication envelope in `public/dashboard/feed.json` from the exact current `main` SHA and, only when that watermark is more than two hours old (or the artifact is missing or invalid), checks that both Autonomous Ingestion and Technology Dashboard Refresh have no active or pending run. It then dispatches the Dashboard workflow on `main` with `skip_external_discovery=true`. That recovery, every hourly schedule, and every ingestion completion rebuild from the committed ingestion artifacts on GitHub-hosted capacity, so Dashboard freshness does not depend on the Mac runner, battery, or wake state. An explicit manual run with `skip_external_discovery=false` remains available on the Mac for full YouTube and Instagram discovery with exact source-proof gates. Failed, cancelled, and skipped Dashboard runs do not count as freshness and do not consume durable state; only an accepted supervisor dispatch starts the 30-minute cooldown. A recently completed successful Dashboard wake receives a 30-minute silence window because a broad-source outage may deliberately retain the older truthful watermark.
...
- Retry-controller deadline: 335 minutes on hosted Linux and 370 minutes for an authenticated Mac replay. It starts a child only with at least 331 minutes remaining, then allows six minutes after `SIGTERM` before a hard kill.
- Overall ingestion step/job timeouts are 342/360 minutes on hosted Linux and 380/415 minutes for an authenticated Mac replay. This retains controller-finalization, cache-save, and job setup/post-step headroom.
- Public cohort collectors run in parallel. GitHub cohort collectors share a serialized queue to respect public API limits.

A deadline-stopped child can leave a validated snapshot or public checkpoint. Snapshot recovery is on by default, and GitHub Actions keeps collector state outside the checkout-cleaned repository when `RETURNER_INGESTION_STATE_ROOT`, `OPENCLI_HOME`, or `RUNNER_WORKSPACE` is available. Because GitHub-hosted filesystems are ephemeral, a failed or cancelled public run also saves only its configured `RETURNER_INGESTION_STATE_ROOT` to an Actions cache whenever the runner remains available for post-steps; the restore prefix is bound to the exact slot and source SHA, and the next compatible hosted wakeup restores the newest entry. Cache restore/save failures are nonblocking and fall back to fresh collection. The cache never includes `OPENCLI_HOME`, authenticated browser profiles, repository credentials, the publication worktree, or `node_modules`. The next run for the same idempotency key reuses only snapshots that pass the exact campaign, attempt, shard, source, and freshness bindings. Up to the computed terminal mapped-failure budget can publish a clearly labeled degraded refresh; any nonterminal task, an incomplete collector matrix, or zero successful collection rows blocks publication.
