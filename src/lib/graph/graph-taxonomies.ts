...
export function enrichEvidenceTopics(item: EvidenceItem): EvidenceItem {
  ...
  // Manual corrections are durable decisions.  A scheduled rules backfill may
  // write a newer taxonomy version, but it must never erase an active override.
  if (item.topicClassification?.method === "manual" && existingTopics.length > 0) {
  ...
  // Older snapshots stored only a flat `topics` list.  Those values were
  // generated by the retired taxonomy, not maintainer overrides, and must be
  // reclassified rather than accidentally promoted to curated truth.
  const explicitTopics = item.topicClassification?.method === "curated"
    ? existingTopics
    : [];
